...
const getNextFeedToRow = `-- name: GetNextFeedToRow :one
SELECT id, created_at, updated_at, name, url, user_id, last_fetched_at FROM feeds 
ORDER BY last_fetched_at ASC NULLS FIRST 
LIMIT 1
`

func (q *Queries) GetNextFeedToRow(ctx context.Context) (Feed, error) {
	row := q.db.QueryRowContext(ctx, getNextFeedToRow)
	var i Feed
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
